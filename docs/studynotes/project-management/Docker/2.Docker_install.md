...
permalink: /project-management/Docker/2.Docker_install
categories:
  - Docker
tags:
  - Docker
...
![image](./assets/image.3b1ygz3skt80.png)
...
相当于容器的“源代码”，<mark>**docker镜像文件类似于Java的类模板，而docker容器实例类似于java中new出来的实例对象**</mark>。
...
## 1.3.Docker平台架构图解(入门版)

![image](./assets/image.1dhfdk9cfh1c.png)

## 1.4.Docker平台架构图解(架构版)
...
![image](./assets/image.6zaea6xum9k0.png)
...
<img src="./assets/image.7hh30r9dad40.png" alt="image" style="zoom:67%;" />
...
保证CentOS能够正常上网。`192.168.233.130`
...
![image](./assets/image.1zbgqzzakxeo.png)

操作步骤是统一的，不存在不同用户不同的操作步骤，我已经放到下面代码块：

`注意：https://ecfqrb5s.mirror.aliyuncs.com替换成自己的`

```sh
sudo mkdir -p /etc/docker
sudo tee /etc/docker/daemon.json <<-'EOF'
{
  "registry-mirrors": ["https://ecfqrb5s.mirror.aliyuncs.com"]
}
EOF
```
...
![image](./assets/image.4fq3ksgij3g.png)

## 1.11.为什么Docker会比VM虚拟机快
...
![image](./assets/image.4xh8wae98fk0.png)
...
![image-20221117095826612](./assets/image-20221117095826612.png)
