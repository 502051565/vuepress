...
访问：http://192.168.233.129:9200/  出现下面信息说明安装启动成功

![image-20221128104732787](./assets/7.glsc_elasticsearch/image-20221128104732787.png)
...
```shell
docker run --name kibana -e ELASTICSEARCH_HOSTS=http://192.168.233.129:9200 -p 5601:5601 -d kibana:7.6.2
```
...
查看elasticsearch版本信息： http://192.168.233.129:9200/ 

```json
{
  "name" : "ea54d0d635e4",
  "cluster_name" : "elasticsearch",
  "cluster_uuid" : "ZtRtS1-oS763O8VoPy_KXw",
  "version" : {
    "number" : "7.6.2",
...
显示elasticsearch 节点信息http://192.168.233.129:9200/cat/nodes ，
...
访问Kibana： http://192.168.233.129:5601/app/kibana 

![image-20200501192629304](assets/7.glsc_elasticsearch/image-20200501192629304.png)
...
http://192.168.233.129:9200/_cat/nodes
```
...
http://192.168.233.129:9200/_cat/health
```
...
http://192.168.233.129:9200/_cat/master
```
...
http://192.168.233.129:9200/_cat/indices
```
...
```sh
http://192.168.233.129:9200/customer/external/1
{
    "name":"John name"
}
```

![image-20200501194449944](./assets/7.glsc_elasticsearch/image-20200501194449944.png)
...
```sh
http://192.168.233.129:9200/customer/external
{
    "name":"John name"
}
```

<img src="./assets/7.glsc_elasticsearch/image-20200501195619925.png" alt="image-20200501195619925" style="zoom: 52%;" />

再次使用POST插入数据，仍然是新增的：

<img src="./assets/7.glsc_elasticsearch/image-20200501195732492.png" alt="image-20200501195732492" style="zoom: 80%;" />



添加数据的时候，指定ID，会使用该id，并且类型是新增：

<img src="./assets/7.glsc_elasticsearch/image-20200501200048361.png" alt="image-20200501200048361" style="zoom: 66%;" />

再次使用POST插入数据，类型为updated

<img src="./assets/7.glsc_elasticsearch/image-20200501200132199.png" alt="image-20200501200132199" style="zoom:67%;" />
...
```sh
#if_seq_no的值为http://192.168.233.129:9200/customer/external/1查出来的_seq_no的值
http://192.168.233.129:9200/customer/external/1?if_seq_no=1&if_primary_term=1
 {
 "name":"1"
 }
```

<img src="./assets/7.glsc_elasticsearch/image-20200501212224983.png" alt="image-20200501212224983" style="zoom: 61%;" />

 （2）将name更新为2，更新过程中使用seq_no=6

```sh
http://192.168.233.129:9200/customer/external/1?if_seq_no=6&if_primary_term=1 
```

<img src="./assets/7.glsc_elasticsearch/image-20200501213047499.png" alt="image-20200501213047499" style="zoom: 60%;" />
...
![image-20200501212924094](./assets/7.glsc_elasticsearch/image-20200501212924094.png)
...
```sh
 http://192.168.233.129:9200/customer/external/1?if_seq_no=7&if_primary_term=1 
```

<img src="./assets/7.glsc_elasticsearch/image-20200501213130001.png" alt="image-20200501213130001" style="zoom:75%;" />

### 4）更新文档

![image-20200501214522818](./assets/7.glsc_elasticsearch/image-20200501214522818.png)

 ![image-20200501215746139](./assets/7.glsc_elasticsearch/image-20200501215746139.png)

（1）POST更新文档，带有_update

```sh
http://192.168.233.129:9200/customer/external/1/_update
```

![image-20200501214810741](./assets/7.glsc_elasticsearch/image-20200501214810741.png)

如果再次执行更新，则不执行任何操作，序列号也不发生变化

![image-20200501214912607](./assets/7.glsc_elasticsearch/image-20200501214912607.png)

POST更新方式，会对比原来的数据，和原来的相同，则不执行任何操作（version和_seq_no）都不变。

 （2）POST更新文档，不带_update

![image-20200501215358666](./assets/7.glsc_elasticsearch/image-20200501215358666.png)
...
<img src="./assets/7.glsc_elasticsearch/image-20200501220559094.png" alt="image-20200501220559094" style="zoom:67%;" />
...
![image-20200501221105476](./assets/7.glsc_elasticsearch/image-20200501221105476.png)
...
![image-20200501233938697](assets/7.glsc_elasticsearch/image-20200501233938697.png)

![image-20200502000221422](assets/7.glsc_elasticsearch/image-20200502000221422.png)

![image-20200502000733594](assets/7.glsc_elasticsearch/image-20200502000733594.png)
...
![image-20200502120921830](./assets/7.glsc_elasticsearch/image-20200502120921830.png)
...
![image-20200502161339291](./assets/7.glsc_elasticsearch/image-20200502161339291.png)
...
![image-20200502170924399](./assets/7.glsc_elasticsearch/image-20200502170924399.png)
...
![image-20200502174825233](./assets/7.glsc_elasticsearch/image-20200502174825233.png)
...
![image-20200502175901959](assets/7.glsc_elasticsearch/image-20200502175901959.png)
...
![image-20200502181432745](./assets/7.glsc_elasticsearch/image-20200502181432745.png)
...
![image-20200502182929583](./assets/7.glsc_elasticsearch/image-20200502182929583.png)
...
[root@node1 plugins]# yum install wget
[root@node1 plugins]# wget  https://github.com/medcl/elasticsearch-analysis-ik/releases/download/v7.6.2/elasticsearch-analysis-ik-7.6.2.zip
[root@node1 plugins]# ll
总用量 4400
-rw-r--r--. 1 root root 4504473 12月  8 2021 elasticsearch-analysis-ik-7.6.2.zip
[root@node1 plugins]# yum install unzip
[root@node1 plugins]# unzip elasticsearch-analysis-ik-7.6.2.zip -d ik
#重启
[root@node1 plugins]# 
```
...
#下载ik7.6.2  wget 安装 yum install wget
...
```shell
[root@0adeb7852e00 elasticsearch]# yum install unzip
...
* 因为ik已经挂载到外面，所有直接修改/mydata/elasticsearch/plugins/ik/config/IKAnalyzer.cfg.xml即可
...
<properties>
	...
	<!--用户可以在这里配置远程扩展字典 -->
	<entry key="remote_ext_dict">http://192.168.233.129/es/fenci.txt</entry> 
	<!--用户可以在这里配置远程扩展停止词字典-->
...
http://192.168.233.129/es/fenci.txt，这个是nginx上资源的访问路径

在运行下面实例之前，需要安装nginx（安装方法见安装nginx），然后创建“fenci.txt”文件，内容如下：

```shell
echo "尚硅谷" > /mydata/nginx/html/es/fenci.txt 
```
...
* 更改nginx文件夹
  ...
  [root@node1 mydata]# mv nginx conf
  [root@node1 mydata]# mkdir nginx
  [root@node1 mydata]# mv conf nginx/
...
```sh
[root@node1 html]# mkdir es
[root@node1 html]# cd es
#在es文件夹下创建fenci.txt文件
echo "尚硅谷" > /mydata/nginx/html/es/fenci.txt
```
...
![image-20200511074437763](assets/7.glsc_elasticsearch/image-20200511074437763.png)
...
![image-20200511111618183](assets/7.glsc_elasticsearch/image-20200511111618183.png)

测试后：

![image-20200511112025327](assets/7.glsc_elasticsearch/image-20200511112025327.png)
